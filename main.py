...
def free_play_normal(segment):
    ...
    while 1:
        ...
        for platform in platforms:
            ...
            if player.rect[0] - platform.rect[0] > 1100:
        ...
        for bullet in bulletss:
            ...
            new_bullet.append(pygame.Rect(bullet.pos[0], bullet.pos[1], 64, 32))
        ...
        for p in drawn_entities:
            ...
            for bullet in new_bullet:
                if bullet.colliderect(p.rect) and "PlatEnemy" not in temp:
                    bulletss.pop(count)
            ...
            for bullet in BULLETS:
                for bullet2 in new_bullet:
                    if bullet2.colliderect(bullet):
                        new_bullet.remove(bullet2)
                        BULLETS.remove(bullet)
                        bulletss.pop(count)
                        count2 = 0
                    count2 += 1
            ...
            if "PlatEnemy" in temp:
                if player.rect[0] - p.rect[0] > 1090:
                    enemy_list.remove(p)
            if "PlatEnemy2" in temp:
                if player.rect[0] - p.rect[0] > 1090:
                    enemy_list2.remove(p)
            ...
            if current_time - previous_time > randint(2000, 3000):
                for enemy in enemy_list:
                    ...
                    bulletss.append(EnemyBullet(*pos))
                    shoot_sound()
...
def main(Map):
    ...
    while 1:
        ...
        for platform in platforms:
            if (platform.rect[0]) - player.rect[0] < 1300:
                drawn_entities.append(platform)
            if player.rect[0] - platform.rect[0] > 800:
                drawn_entities.remove(platform)
        ...
        screen.blit(player.image,
                    pygame.Rect(player.rect[0] + entities.cam[0], player.rect[1] + entities.cam[1], 32, 32))
